...
rng.seed(seed)
echo "\n------------------------------------------------------\n"
echo "test_io xoshiro512** seed: ", seed
  ...
  suite "IO - BigInt" & " [" & $WordBitwidth & "-bit mode]":
    test "Parsing raw integers":
      block: # Sanity check
        ...
        let big = BigInt.fromRawUint(x_bytes, cpuEndian)
    ...
    test "Parsing and dumping round-trip on uint64":
      block:
        ...
        let x = 1'u64 shl 63
        let x_bytes = cast[array[8, byte]](x)
        let big = BigInt.fromRawUint(x_bytes, littleEndian) # It's fine even on big-endian platform. We only want the byte-pattern

        var r_bytes = canary(array[8, byte])
        let ok = exportRawUint(r_bytes, big, littleEndian)
        check ok
